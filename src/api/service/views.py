...
class ServiceViewSet(viewsets.ModelViewSet):
    ...
    def get_barber_services(self, request, pk=None):
        barber = get_object_or_404(User, pk=pk, roles__name='Barber')

        servicetypes = ServiceType.objects.filter(barber = barber)
        
        # service = ServiceType.objects.filter(service_type = servicetypes)
